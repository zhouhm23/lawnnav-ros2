...
### 5.29
今天任务是尽快完成预实验，确保所有实验成功获得至少一组有效数据。除非严重错误导致实验无法完成，否则不进行任何代码修改、优化或重构，数据异常可多次实验解决。
1. 实验结果保存出错，紧急手动保存数据->修正表头错误->测试通过
slam预实验成功，获得第一组有效数据->已提交代码

### 5.30
今天目标是完成覆盖预实验
1. 路径规划非常差，原因是膨胀层太大，但是膨胀层太小又容易碰障碍物->过去成功经验为建图时和覆盖时nav参数不一样，因为nav路径规划不会考虑车宽，只看膨胀层，因此需要设置成0.2，而且slam实验路程短，需要clearing: False否则不能避障；覆盖时路径覆盖包会看膨胀层和车宽，导致规划路径小了很多，而且路径长需要clearing为True避免障碍物一直膨胀->使用两套配置，覆盖率明显提高->测试通过
2. 路径覆盖任务结束时test_coverage_comparison不能正常结束->可加一个输入F回车正常结束->测试通过
3. 今晚10次成功实验，出现大量随机问题，无成功完成任务：
   1. 规划完路径车未运动，看输出发现车显示遇到障碍物，可rviz里并没有看到任何代价->设置min_range: 0.05->还是高频出现（3/10）
   2. 图像跟踪效果差->标识码背景不好，明天加个A4背景，提高识别率
   3. 突然odom_raw崩溃不更新，原因未知
   4. 突然小车关机，原因未知
   5. 小车突然定位大偏差，特别是角度->因为建图时周围墙壁有点歪，回环检查不严格导致错误修正->收紧回环修正参数，有点缓解
   6. 小车突然断开网络->可能需要改成小车连电脑，但ssh要重新配置，风险有点大
   7. 覆盖包缺少最小覆盖面积，会出现小于车面积大小的cell->需要改源码，风险大，也可以想办法调参解决，特别是coverage_clearance
   如果这些随机问题难以解决，那可能需要多分支准备两份工作空间，但这会丢失大量改进工作，只作为保底选项

### 6.1
目标解决5.30的问题，完成覆盖预实验
run1：启动后小车原地转圈，不前往目标点->b790f32把RTAB-Map的Reg/Strategy从ICP改成了纯视觉，同时把Vis/MinInliers提到36、Vis/MaxFeatures提到1600->太严格导致位置失效，无法启动->回退->测试通过
run2：运行到中间时小车被回环修正带偏，最后撞墙后出现Rejected loop closure，定位失败，原因多半还是rtabmap_params.yaml配置不对，已回退默认参数->这次没崩溃，但不能确定该随机问题再不出现
run3：
细胞分解会出现小面积区域（比车面积还小），导致绕大路径且小面积容易碰障->给路径覆盖包加一个最小cell面积参数->测试通过
视觉模式下小车会把看不到的障碍物清除，这是因为nav订阅scan是当成全向雷达来处理，它会以为相机视野外没障碍物->看看有没有合适的plugin，否则可能要自己写->设置inf_is_valid: false防止清除视野外的障碍物。失败做法：voxel_layer不能用点云，clearing设为false不行->测试通过
run4：
小车运行过程中突然关机->没电了
...
出现随机问题：
1. 小车突然一直旋转或前进，无视导航命令，且关闭程序时速度不归0（正常关闭都会发布0命令，且查询到cmd_vel的订阅者仍存在），但重启nav就能停止->出现次数：4
   已查明原因：tf和nav的时间差距过大，导致nav崩溃->尝试调大transform_tolerance:0.5to1->反而启动失败，显示无法初始位置->回退原版，不做改动，仅靠多次运行排除这个偶然问题，也可等小车电量高时就不会出现这个问题 
2. 小车突然短暂断开wifi链接，然后很快恢复，但这会断开摄像头链接（远程摄像头没自动续连）->出现次数：1

# 6.6 
1. 昨天的问题1在run1 run2连续出现，必须想办法解决，日志为logs/coverage/lidar_baseline_run0_nav.log
2. 修改：
文件	参数	改前	改后	省 CPU
slam.yaml	throttle_scans	1	2	slam_toolbox -50%
slam.yaml	transform_publish_period	0.02s	0.05s	TF 发布 -60%
slam.yaml	scan_buffer_size	20	10	匹配搜索 -50%
nav2_params_lidar.yaml	local update/publish	5/2 Hz	3/1 Hz	costmap
nav2_params_lidar.yaml	global update/publish	1/1 Hz	0.5/0.5 Hz	costmap
nav2_params_lidar.yaml	behavior cycle	10 Hz	5 Hz	BT 调度
nav2_controller_lidar.yaml	controller_frequency	10 Hz	5 Hz	控制器
ekf_lidar.yaml	frequency	100 Hz	50 Hz	EKF -50%
测试结果：非常有效，但还是存在一些网络波动，但能完成任务