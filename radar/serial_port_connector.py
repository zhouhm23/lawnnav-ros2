...
class SerialPortConnector:
    ...
    # 缓冲区限制 (降低到 1MB 以防止延迟过大)
    MAX_DATA_LEN = 1 * 1024 * 1024  # 1 MB
    ...
    def __init__(self, port_name: str = 'COM13', baud_rate: int = 921600, 
        ...
        self.stats = {
            ...
            'bytes_processed': 0,
            'sync_attempts': 0,
            'reclaims': 0,  # 缓冲区回收计数（内存回收历史）
            'reconnects_on_high_buffer': 0
        }
        ...
        # 高缓冲区策略
        # 当缓冲区长度超过阈值比例时，触发重连而不是回收（默认启用）
        self.reconnect_on_high_buffer = True
        self.buffer_high_threshold_ratio = 0.9
        # 如果你希望改为回收（而不是重连），可设置为 False
        self.buffer_reclaim_keep_ratio = 0.1
        # 外部控制开关路径：如果设置了该文件路径，存在且内容为 '1'/'true' 则允许重连
        # 方便外部脚本通过写文件来启/禁重连策略（例如 echo 1 > /tmp/radar_reconnect_enable）
        self.external_reconnect_control_path = None
            ...
            try:
                with self._lock:
                    ...
                    if mcu_data_len > self.MAX_DATA_LEN:
                        ...
                        # 长度字段异常通常意味着已丢字节/不同步，直接跳到下一个帧头更快
                        self.stats['mcu_len_errors'] = self.stats.get('mcu_len_errors', 0) + 1
                        self._skip_garbage()
                        continue
                    ...
                    if not self._validate_mcu_tail(total_len):
                        # 帧尾错误多数来自边界错位/丢字节，滑窗1字节会很慢且更易二次不同步
                        self.stats['mcu_tail_errors'] = self.stats.get('mcu_tail_errors', 0) + 1
                        self._skip_garbage()
                        continue
                    ...
                    if len(radar_seg) != mcu_data_len:
                        ...
                        self.stats['mcu_segment_len_errors'] = self.stats.get('mcu_segment_len_errors', 0) + 1
                        self._skip_garbage()
                        continue
    ...
    def _is_reconnect_allowed_by_external(self) -> bool:
        """检查外部控制文件（若设置）以决定是否允许自动重连。
        若未设置控制文件则返回当前 internal 配置 self.reconnect_on_high_buffer。
        控制文件中允许的真值：'1', 'true', 'yes', 'on'（不区分大小写）。"""
        if not self.external_reconnect_control_path:
            return bool(self.reconnect_on_high_buffer)
        try:
            with open(self.external_reconnect_control_path, 'r') as fh:
                val = fh.read().strip().lower()
            return val in ('1', 'true', 'yes', 'on')
        except Exception as e:
            self.logger.debug(f'读取外部重连控制文件失败: {e}，回退使用内部配置')
            return bool(self.reconnect_on_high_buffer)

    def set_external_reconnect_control(self, path: str):
        """设置外部控制文件路径（传 None 可禁用外部控制）。"""
        self.external_reconnect_control_path = path
        self.logger.info(f'外部重连控制文件路径设置为: {path}')

    def set_reconnect_on_high_buffer(self, enabled: bool):
        """运行时修改是否在高缓冲时触发重连的策略。"""
        self.reconnect_on_high_buffer = bool(enabled)
        self.logger.info(f'运行时设置 reconnect_on_high_buffer = {self.reconnect_on_high_buffer}')

    def _handle_high_buffer(self) -> bool:
        """当缓冲区接近上限时，根据配置执行操作。
        如果启用了重连，会尝试调用 reconnect() 并返回 True 表示已处理；
        否则执行回收并返回 True 表示已回收；返回 False 表示未达到阈值。
        """
        try:
            threshold = int(self.MAX_DATA_LEN * self.buffer_high_threshold_ratio)
            if len(self.buffer) >= threshold:
                old_len = len(self.buffer)
                # 优先策略：在外部允许的情况下进行重连
                if self._is_reconnect_allowed_by_external():
                    self.logger.warning(
                        f'缓冲区达到高阈值 ({len(self.buffer)} bytes >= {threshold}), 外部允许重连，将启动重连...'
                    )
                    self.stats['reconnects_on_high_buffer'] = self.stats.get('reconnects_on_high_buffer', 0) + 1
                    try:
                        self.reconnect()
                    except Exception as e:
                        self.logger.debug(f'重连试图在高缓冲区处理时失败: {e}')
                    # 清理缓冲区以避免下一轮立即触发
                    self.buffer.clear()
                    return True
                else:
                    # 外部不允许重连，退回到回收策略
                    keep_size = int(self.MAX_DATA_LEN * self.buffer_reclaim_keep_ratio)
                    self.buffer = self.buffer[-keep_size:]
                    self.stats['reclaims'] = self.stats.get('reclaims', 0) + 1
                    self.logger.warning(
                        f'外部禁止重连，执行缓冲区回收: 原长={old_len} bytes, 阈值={threshold} bytes, 保留尾部={len(self.buffer)} bytes'
                    )
                    return True
        except Exception as e:
            self.logger.debug(f'处理高缓冲区时出错: {e}')
        return False

    def _check_buffer_overflow(self) -> bool:
        """检查并处理缓冲区状态：先尝试高缓冲策略，再处理真正溢出情况"""
        # 1) 当缓冲区达到高阈值时，执行配置的处理（重连或回收）
        if len(self.buffer) >= int(self.MAX_DATA_LEN * self.buffer_high_threshold_ratio):
            handled = self._handle_high_buffer()
            if handled:
                # 上层循环会继续下一次读取
                return True
    ...
    def _skip_garbage(self):
        """跳过当前错误数据，直接搜索缓冲区中下一个MCU帧头位置。

        在高速串口场景下，一旦丢字节导致不同步，逐字节滑窗会非常慢，
        且容易在噪声/数据段中误伤；直接定位下一个帧头更稳健。
        """
        # 从当前位置+1开始搜寻 'ICLH'
...
if __name__ == "__main__":
    ...
    # ==================== 用户配置区 ====================
    PORT = 'COM13'               # 根据实际修改（Windows: COMx, Linux: /dev/ttyUSBx）
    BAUD = 921600                # 波特率
