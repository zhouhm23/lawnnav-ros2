...
class RadarSignalConfig:
    ...
    def __init__(self):
        ...
        # --- Headless / 小车部署相关: 映射与处理节奏配置 ---
        # 是否启用 GUI（默认 True）。在小车部署时设置为 False
        self.ENABLE_GUI = True
        # 线性距离分辨率 (米/FFT bin)，或者使用 BIN_DISTANCE_ARRAY 手动设置每个 bin 对应距离
        self.BIN_DISTANCE_RESOLUTION = None
        self.BIN_DISTANCE_ARRAY = None
        self.DIST_OFFSET = 0.0
        # 目标处理间隔（毫秒），当输入帧率高于该值时会节流处理
        self.PROCESSING_TARGET_INTERVAL_MS = 100
...
class RadarSystem:
    def __init__(self, config: RadarSignalConfig):
        ...
        self.running = False
        # Headless / callback related attributes
        self.callback = None
                    ...
                    if data:
                        ...
                        if processed:
                            ...
                            if self.config.ENABLE_GUI:
                                ...
                                # headless mode: adaptive processing based on target interval
                                now = time.time()
                                if self.last_processed_time is None:
                                    self.last_processed_time = now
                                elapsed_ms = (now - self.last_processed_time) * 1000.0
                                target = self.config.PROCESSING_TARGET_INTERVAL_MS
                                if elapsed_ms >= target:
                                    self._process_and_callback(cube)
                                    self.last_processed_time = now
                    else:
                        time.sleep(0.001)
    ...
    # ---------- Headless helper & callback ----------
    def register_callback(self, callback_fn):
        """Register a callable to receive per-frame dictionaries in headless mode."""
        self.callback = callback_fn

    def _bin_to_distance(self, bin_idx, n_samples):
        """Convert fft/bin index to distance using config mapping (array or resolution)."""
        # prefer explicit array mapping
        if self.config.BIN_DISTANCE_ARRAY is not None:
            arr = np.asarray(self.config.BIN_DISTANCE_ARRAY)
            idx = int(np.mod(int(round(bin_idx)), n_samples))
            if idx < len(arr):
                return float(arr[idx])
            else:
                return float(arr[idx % len(arr)])
        elif self.config.BIN_DISTANCE_RESOLUTION is not None:
            idx = int(np.mod(int(round(bin_idx)), n_samples))
            return idx * float(self.config.BIN_DISTANCE_RESOLUTION) + float(self.config.DIST_OFFSET)
        else:
            return None

    def _process_and_callback(self, cube_raw):
        now = time.time()
        self._frame_count += 1
        cube_clean, anchor_map, comm_res, anchor_info, comm_bits = self.processor_anchor.process(cube_raw)
        freq_bin = anchor_info.get('freq_bin', 0)
        angle = anchor_info.get('angle_deg', 0.0)
        amp = anchor_info.get('amp', 0.0)
        distance = self._bin_to_distance(freq_bin, cube_raw.shape[0])
        # frame rate estimate
        fps = None
        if self._prev_callback_time is not None:
            dt = now - self._prev_callback_time
            if dt > 0:
                fps = 1.0 / dt
        ...
        data = {
            ...
            'amplitude': amp,
            'comm_bits': comm_bits,
            'offsets': comm_res.get('offsets'),
            'frame_rate': fps,
            'n_chirps': cube_raw.shape[1]
        }
        if self.callback:
            try:
                self.callback(data)
            ...
            logger.info(f"Headless frame: bin={freq_bin}, dist={distance}, angle={angle}, bits={comm_bits}")
    ...
    def run(self):
        ...
        # If GUI is enabled and figure exists, run animation loop.
        if self.config.ENABLE_GUI and hasattr(self, 'fig'):
            ...
            # Headless mode: block until interrupted, let acquisition thread call callbacks.
            try:
                while self.running:
                    time.sleep(0.1)
                ...
                # Give acquisition thread a moment to exit
                try:
                    t.join(timeout=1.0)
                except Exception:
                    pass
...
if __name__ == "__main__":
    ...
    # headless / car deployment
    config.ENABLE_GUI = False
    config.BIN_DISTANCE_RESOLUTION = 0.05  # meters per FFT bin (example)
    config.DIST_OFFSET = 0.0
    config.PROCESSING_TARGET_INTERVAL_MS = 100
    # ============================
    ...
    def print_callback(data):
        ...
        bits = data.get('comm_bits')
        print(f"[{ts:.3f}] Bin:{freq_bin}, Dist:{distance if distance is not None else 'N/A'} m, Angle:{angle:.1f}°, Bits:{bits}")

    app.register_callback(print_callback)
