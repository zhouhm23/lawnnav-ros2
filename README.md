...
## 项目简介
本项目是基于树莓派嵌入式平台的割草机自主导航系统，依托 ROS2 框架构建，集成 RTAB-Map 视觉 SLAM、Nav2 导航栈与全区域覆盖规划算法，实现从环境建图、路径规划到运动执行的完整工程链路。
...
```
src/
├── path_coverage/          # 路径覆盖规划包（基于开源项目二次开发）
├── nav2_config/            # Nav2 导航配置
├── rtabmap_config/         # RTAB-Map SLAM 配置
├── color_overlay/          # 地面颜色识别包
├── coverage_evaluator/     # 评估工具链包
└── launch/                 # 系统启动脚本
```
...
## 开发日志
本项目开发日志基于完整 git 提交历史整理，详细记录了从最小闭环搭建到工程化升级的全流程代码改动，具体可参考仓库提交记录。
