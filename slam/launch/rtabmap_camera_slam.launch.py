#!/usr/bin/env python3
"""
rtabmap_camera_slam.launch.py — (a) 单相机 RTAB-Map 建图
...
TF 树:
  ...
  提供者:
    map→odom:      RTAB-Map (visual SLAM)
    odom→base:     controller (robot_localization / odometry)
    base→depth:    depth_camera.launch.py (static TF + camera_info)
    base→base_ft:  robot_state_publisher (URDF)
...
def launch_setup(context):
    ...
    depth_camera_topic = '/depth/image'.format(topic_prefix)
    depth_camera_info = '/rgb/camera_info'.format(topic_prefix)
    rgb_camera_topic = '/rgb/image'.format(topic_prefix)
    ...
    rtabmap_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(slam_package_path, 'launch/include/rtabmap_camera.launch.py')),
        launch_arguments={
